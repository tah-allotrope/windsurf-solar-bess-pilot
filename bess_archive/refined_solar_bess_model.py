...
class SystemParameters:
    ...
    charge_limit_kw: float = 20000.0  # Max charge power
...
class BatteryStorage:
    """Battery Energy Storage System operations - matching Excel logic exactly"""
    
    def __init__(self, params: SystemParameters):
        self.params = params
        
    def simulate_battery_operation_excel_style(self, 
                                            load_kw: pd.Series,
        ...
        Returns: (discharge_power_kw, charge_power_kwh, soc_kwh, discharge_flag)
        """
        hours = len(load_kw)
        ...
        for hour in range(hours):
            ...
            # Calculate excess solar available for charging
            if solar_gen_kw.iloc[hour] > 0:
                excess_solar = min(solar_gen_kw.iloc[hour], load_kw.iloc[hour])
            else:
                excess_solar = 0.0
            
            # Calculate headroom and charge limit
            headroom = self.params.bess_capacity_kwh - current_soc
            charge_limit_kwh = min(self.params.charge_limit_kw, headroom)
            
            # Determine discharge condition flag (Excel logic)
            # Based on analysis: discharge when load > demand_target AND battery has charge
            if net_load_after_solar > demand_target_kw and current_soc > 0:
                discharge_flag[hour] = 1
            else:
                discharge_flag[hour] = 0
            
            # Charging logic (Excel style)
            if excess_solar > 0 and headroom > 0:
                # Charge with excess solar, limited by charge limit
                available_charge = min(excess_solar, charge_limit_kwh)
                charge_energy[hour] = available_charge
                current_soc += available_charge * self.params.battery_efficiency
            
            # Discharging logic (Excel style)
            if discharge_flag[hour] == 1 and current_soc > 0:
                # Discharge to meet demand target
                needed_discharge = min(net_load_after_solar - demand_target_kw, 
...
class FinancialCalculator:
    ...
    def calculate_energy_costs_excel_style(self, 
...
class SolarBESSModel:
    """Main Solar + BESS financial model - refined to match Excel exactly"""
    ...
    def run_simulation_excel_style(self, 
                                hourly_data: pd.DataFrame) -> Dict:
        """
        Run complete simulation matching Excel logic exactly
        ...
        # Simulate battery operation using Excel logic
        discharge_kw, charge_kwh, soc_kwh, discharge_flag = self.battery.simulate_battery_operation_excel_style(
            hourly_data['Load_kW'], 
            solar_gen_kw, 
        ...
        costs = self.finance.calculate_energy_costs_excel_style(
            hourly_data['Load_kW'],
            final_grid_load,
            hourly_data['FMP']
        )
    ...
    def run_lifetime_analysis_excel_style(self, hourly_data: pd.DataFrame) -> Dict:
        ...
        for year in range(1, self.params.project_life_years + 1):
            ...
            year_results = year_model.run_simulation_excel_style(hourly_data)
...
if __name__ == "__main__":
    print("Refined Solar + BESS Financial Model (Excel Style)")
    print("="*60)
    ...
    print(f"Charge Limit: {model.params.charge_limit_kw:.0f} kW")
